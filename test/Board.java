...
public class Board
{
    ...
    // check if the board is currently legal to place the new word
    public boolean boardLegal(Word word)
    {
		...
		if (!restsOnExistingTile(word) && wordsCounter > 0)
        { // the first if it passed the previous conditions word
																// must be legal
			return false;
		}
    ...
    {
        // means the there is already first word is not on a star
		if (wordsCounter > 0 && tiles[7][7] != null)
        {
			return true;
		}
	...
	private boolean restsOnExistingTile(Word word)
    {
        ...
        // check the relevant situations, if the word is vertical or not
		if (word.isVertical())
        { 
    ...
    public int checkWordBonus(int row, int col)
    {
        ...
        else if (row == 7 && col == 7 && wordsCounter == 1)
        { // star bonus for the first word
            return 2;
        }
            ...
            return 3; // 3 word bonus for a red tile
                ...
                {
                    return 2; // 2 word bonus for a yellow tile
                }
        return 1; // no bonus
    }
